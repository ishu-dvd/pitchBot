...
Unlike the retrieval runners, `run-speech` gates **fail closed and set the process exit code**. It applies a suite-aware gate (`speech_gates_pass`) that fails unless the run corresponds to the reviewed suite, every case and every required metric is present (per-case precision/recall/F1 and real-time factor, per-slice/mean/min F1, real-time factor, peak allocation), all cases passed, and every gating metric met its threshold. A failing gate prints `artifact-gates=fail` **and returns a non-zero exit code**, so CI can actually fail on a regression. This is deliberately stricter than the shared `EvaluationRun.gates_pass()`, which is non-suite-aware (it passes on any single passing metric even with the VAD metrics absent) and which the other runners still return `0` on — repairing that shared machinery is a separate reviewed change (see PROGRESS PR 22 "Deferred").
