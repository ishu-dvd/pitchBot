...
| Recall as an authority channel | Retrieved prior claims steer the reply, classification, or an action a safety refusal just blocked | Recall runs after the durable commit and is never an input to generation, extraction, or policy; it is skipped on any safety signal, on non-continuing dispositions, and on durable replay; failures degrade to no recall |
| Recall as an exfiltration channel | A crafted turn is used to dump journal internals into the browser or the timeline | Single-lead scope; superseded claims filtered twice; `fact_id`/`source_span_ids` never leave the process; bounded `top_k`/deadline budgets; no timeline event is appended, so advisory data cannot evict transcript events, and the raw query is never journaled |
| Spoken safety bypass | A hostile instruction is spoken rather than typed to dodge a text-only control | A transcribed utterance is submitted through the same `process_turn` path as a typed turn; there is no speech-only entry point to the engine, and every disclosure, safety, consent, and policy check applies unchanged |
| Fabricated transcript | A failed, empty, or low-confidence transcription is rendered as something the buyer said | No text is emitted unless transcription succeeded above the confidence floor; empty, low-confidence, oversized, and failed utterances report a machine-readable outcome with no text and create no turn |
| Audio retention drift | Buffering audio for transcription quietly turns `audio_retained=false` into a false statement | Audio is buffered only for the utterance in flight, capped at 2 MiB, released immediately after transcription, and never written to disk, journaled, placed in a timeline event, or echoed back |
| Held-floor denial of service | A stream that never falls silent keeps an utterance open and accumulates audio | Endpointing is bounded by `max_utterance_ms`; the audio buffer is byte-capped and fails the utterance closed; abandoned utterances release their audio and clear the oversize latch, so repeated noise cannot poison the cap; the agent floor is reclaimed after `agent_floor_ms`; per-message size, browser queue depth, and reconnect caps are unchanged |
| Control-frame injection | A text frame on the audio socket is used to drive server state or reach a parser | The only accepted text frame is the literal `playback-finished`; nothing is parsed, and any other text frame closes the connection with a policy violation |
...
- Spoken input is buyer text with extra steps: it is endpointed, transcribed, then passed through the identical turn path, so injection, extraction, and opt-out detection apply to a spoken turn exactly as to a typed one.
